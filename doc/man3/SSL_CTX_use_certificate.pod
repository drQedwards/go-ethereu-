...
key types (for example RSA, ECDSA, Ed25519). Certificates and
private keys are stored internally by key type; loading a new
certificate or private key replaces only an existing item of the
same type and does not affect other configured types.
...
private key being assigned via SSL_CTX_use_PrivateKey(), etc. This
function may be called multiple times to configure multiple
...
will be checked. For example, if the last item installed was an RSA
certificate or key, the RSA key/certificate pair will be checked.
SSL_check_private_key() performs the same check for B<ssl>. If no
key/certificate was explicitly added for this B<ssl>, the last item
added into B<ctx> will be checked.
