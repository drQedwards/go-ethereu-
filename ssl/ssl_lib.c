...
void ssl_set_masks(SSL_CONNECTION *s)
{
    ...
    if (rsa_enc || rsa_sign || (ssl_has_cert(s, SSL_PKEY_RSA_PSS_SIGN) && pvalid[SSL_PKEY_RSA_PSS_SIGN] & CERT_PKEY_EXPLICIT_SIGN && (SSL_version(&s->ssl) == (SSL_is_dtls(&s->ssl) ? DTLS1_2_VERSION : TLS1_2_VERSION))))
        mask_a |= SSL_aRSA;
    ...
    if (pvalid[SSL_PKEY_ECC] & CERT_PKEY_RPK)
        mask_a |= SSL_aECDSA;
    if (SSL_version(&s->ssl) == (SSL_is_dtls(&s->ssl) ? DTLS1_2_VERSION : TLS1_2_VERSION)) {
    ...
    if (!(mask_a & SSL_aECDSA) && ssl_has_cert(s, SSL_PKEY_ED25519)
        ...
        && (SSL_version(&s->ssl) == (SSL_is_dtls(&s->ssl) ? DTLS1_2_VERSION : TLS1_2_VERSION)))
        mask_a |= SSL_aECDSA;
